...
const docs = await client.fetch(`*[_type == "destination" && slug.current == "geneva"]{_id, _type, title}`);
console.log("Existing docs:", JSON.stringify(docs));

// Create draft if missing, then patch
let draftId = "drafts.destination.geneva";

// Try to fetch draft
try {
  const draft = await client.fetch(`*[_type == "destination" && _id == $id]{_id}`, { id: draftId });
  console.log("Draft exists:", JSON.stringify(draft));
} catch (e) {
  console.log("Draft not found, creating...");
...
// Build full body with images inserted before target H2 headings
const body = [
  // --- Getting the Most Out of Geneva ---
  ...
  { _type: "block", _key: "hl001", style: "normal", markDefs: [], children: [
    { _type: "span", _key: "s4a", text: "HIGHLIGHT: Geneva Is Smaller Than You Think — ", marks: ["strong"] },
    { _type: "span", _key: "s4b", text: "The lake, Old Town, and Cornavin main station are all within a 20-minute walk of each other.", marks: [] }
  ]},
  // Map infographic inserted before intro paragraph describing location
  ...
  // --- What Geneva Is Best For --- (image inserted BEFORE)
  ...
  // --- Who Should Stay in Geneva --- (image inserted BEFORE)
  ...
  // --- How Geneva Fits into a Switzerland Itinerary --- (image inserted BEFORE)
...
// Patch the draft
const result = await client.patch(draftId).set({ body, image: heroImage }).commit();
console.log(JSON.stringify({
  ...
  heroAttached: true,
  imagesInsertedBeforeH2: [
    { image: "map", beforeHeading: "Getting the Most Out of Geneva" },
    { image: "best", beforeHeading: "What Geneva Is Best For" },
    { image: "who", beforeHeading: "Who Should Stay in Geneva" },
    { image: "itinerary", beforeHeading: "How Geneva Fits into a Switzerland Itinerary" },
  ],
}));
